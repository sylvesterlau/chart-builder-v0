...
interface SemiDonutPagePageProps {
  onBack: () => void;
}
function SemiDonutPage({ onBack }: SemiDonutPagePageProps) {
  const [itemA, setItemA] = useState<number>(0.0),
    [itemALabel, setItemALabel] = useState<string>(""),
    [itemB, setItemB] = useState<number>(0.0),
    [itemBLabel, setItemBLabel] = useState<string>(""),
    [itemC, setItemC] = useState<number>(0.0),
    [itemCLabel, setItemCLabel] = useState<string>(""),
    [itemD, setItemD] = useState<number>(0.0),
    [itemDLabel, setItemDLabel] = useState<string>(""),
    [itemE, setItemE] = useState<number>(0.0),
  ...
  const handleNumericInput = useCallback((setter: (value: number) => void) => {
    return (value: number | null) => setter(value ?? 0.0);
  }, []);
  const handleGenerateButtonClick = useCallback(
    function () {
      ...
      emit("SUBMIT_SEMI_DONUT_CHART_DATA", formData);
    },
    [
      itemA,
      itemALabel,
      ...
      <Stack space="extraSmall">
        ...
        <Columns space="extraSmall">
          ...
          <TextboxNumeric
            ...
            onBlur={() => setItemA(parseFloat(itemA.toFixed(2)))}
            value={String(itemA)}
            placeholder="Value"
          />
        </Columns>
